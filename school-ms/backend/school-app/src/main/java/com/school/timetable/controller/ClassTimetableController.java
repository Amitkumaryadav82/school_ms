...
public class ClassTimetableController {
    ...
    public record GenerateRequest(Long classId,
                                  @JsonAlias({"section","sectionId"}) String sectionId,
                                  boolean lockExisting) {}

    // Grid cell returned to the UI (IDs retained for compatibility; display fields added)
    ...
    // Response expected by the frontend: periodsPerDay + days + grid(dayIdx -> periodNo -> cell)
    ...
    public ResponseEntity<SlotsResponse> getSlots(@RequestParam Long classId,
        ...
        // Resolve section id from grade+section name using exam tables (if section is a letter)
        Long sectionId = resolveSectionIdIfNeeded(classId, section);

    List<TimetableSlot> slots = slotRepo.findByClassIdAndSectionIdOrderByDayOfWeekAscPeriodNoAsc(classId, sectionId);
    ...
    public ResponseEntity<SlotsResponse> generate(@RequestBody GenerateRequest req) {
        ...
        // Optionally retain existing locked slots; if not requested, clear existing timetable
        if (!req.lockExisting()) {
    ...
    for (int d = 1; d <= 5; d++) { // Mon..Fri
            if (!isWorkingDay(mask, d)) continue;
        ...
        if (req.lockExisting()) {
            List<TimetableSlot> existing = slotRepo.findByClassIdAndSectionIdOrderByDayOfWeekAscPeriodNoAsc(req.classId(), resolvedSectionId);
            for (TimetableSlot s : existing) {
                grid.computeIfAbsent(s.getDayOfWeek(), k -> new HashMap<>()).put(s.getPeriodNo(), s);
            }
        }

        // Fetch requirements: which subjects are needed for this class/section and how many weekly periods
    ...
    // Build subject -> eligible teacher ids using intersection of teacher_class_map and teacher_subject_map
        Map<Long, List<Long>> subjectTeachers = new HashMap<>();
    // Ensure we use section letter for the teacher_class_map join (it stores letters like 'A')
        ...
        // Greedy fill with fair day distribution: iterate and spread occurrences across working days
        ...
        // Prepare ordered list of working days (ISO 1..7, we only consider Mon..Fri here)
        List<Integer> workingDays = new ArrayList<>();
        for (int d = 1; d <= 5; d++) {
            if (isWorkingDay(mask, d)) workingDays.add(d);
        }
        ...
        while (!queue.isEmpty()) {
            ...
            // Try to place one occurrence of this subject across working days, starting at rotating index
            for (int di = 0; di < workingDays.size() && !placedThisRound; di++) {
                ...
                for (int p = 1; p <= periodsPerDay && !placedThisRound; p++) {
                    ...
                    if (slot.getSubjectId() != null) continue; // already filled
    ...
    List<TimetableSlot> slots = slotRepo.findByClassIdAndSectionIdOrderByDayOfWeekAscPeriodNoAsc(req.classId(), resolvedSectionId);
    return ResponseEntity.ok(buildGridResponse(periodsPerDay, slots));
    }
    ...
    private SlotsResponse buildGridResponse(int periodsPerDay, List<TimetableSlot> slots) {
        ...
        try {
            jdbc.query(
                ...
                rs -> { teacherNameById.put(rs.getLong(1), rs.getString(2)); }
            );
        } catch (Exception ignore) { /* best-effort */ }
    ...
    private Long resolveSectionIdIfNeeded(Long classId, String section) {
        ...
        // Derive section id via grade levels and sections tables using exam's class name
    ...
    private Integer extractGradeNumber(String className) {
        ...
        try { return Integer.parseInt(normalized); } catch (NumberFormatException e) { return null; }
    }
