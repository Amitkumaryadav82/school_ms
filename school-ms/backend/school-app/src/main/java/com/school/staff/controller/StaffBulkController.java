...
public class StaffBulkController {
    ...
    /**
     * Bulk endpoint for creating or updating multiple staff records using a structured JSON format
     */
    @PostMapping("/bulk")
    @Operation(
        operationId = "bulkCreateStaff",
        summary = "Create or update multiple staff members",
        description = "Bulk creates or updates multiple staff records in one operation"
    )
    @ApiResponses({
        @ApiResponse(
            responseCode = "200",
            description = "Successfully processed staff records",
            content = @Content(
    ...
    @Operation(
        operationId = "bulkImportStaffFromFile",
        summary = "Import staff data from file",
        description = "Bulk imports staff data from a CSV or Excel file"
    )
    @ApiResponses({
    ...
    public ResponseEntity<BulkUploadResponse> bulkImportStaffFromFile(
            @Parameter(description = "CSV or Excel file containing staff data")
    ...
    @Operation(
        operationId = "bulkImportStaffArray",
        summary = "Import staff data from array",
        description = "Legacy endpoint for bulk import of staff as raw JSON array"
    )
    @ApiResponses({
