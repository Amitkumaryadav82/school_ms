  ...
  async error => {
    ...
    if (error.response?.status === 403) {
      ...
      console.error('User from localStorage:', localStorage.getItem('user'));
        if (import.meta.env.DEV) console.error('Permission Error (403) Details:');
        if (import.meta.env.DEV) console.error('Current localStorage token:', localStorage.getItem('token') ? 'Present (first 10 chars): ' +
          localStorage.getItem('token')?.substring(0, 10) + '...' : 'Missing');
        if (import.meta.env.DEV) console.error('User from localStorage:', localStorage.getItem('user'));
      let fullRequestUrl = '';
      ...
      console.error('Request Headers:', error.config?.headers);
        if (import.meta.env.DEV) console.error('Full request URL:', fullRequestUrl);
        if (import.meta.env.DEV) console.error('Request Method:', error.config?.method?.toUpperCase());
        if (import.meta.env.DEV) console.error('Request Headers:', error.config?.headers);
      try {
      ...
      console.error('Response Body:', error.response?.data);
        if (import.meta.env.DEV) console.error('Response Body:', error.response?.data);
    }

    // 500: server error diagnostics
    if (error.response?.status === 500) {
      console.error('Server Error 500 Details:');
        if (import.meta.env.DEV) console.error('Server Error 500 Details:');
      try {
        console.error('Request URL:', error.config?.url);
        console.error('Request Method:', error.config?.method);
        console.error('Request Payload:', JSON.parse(error.config?.data));
        console.error('Response Data:', error.response?.data);
          if (import.meta.env.DEV) console.error('Request URL:', error.config?.url);
          if (import.meta.env.DEV) console.error('Request Method:', error.config?.method);
          if (import.meta.env.DEV) console.error('Request Payload:', JSON.parse(error.config?.data));
          if (import.meta.env.DEV) console.error('Response Data:', error.response?.data);
      } catch (e) {
        console.error('Raw Request Data:', error.config?.data);
          if (import.meta.env.DEV) console.error('Raw Request Data:', error.config?.data);
      }
    }
    ...
    if (error.response?.status === 400) {
      console.error('Validation Error Details:');
        if (import.meta.env.DEV) console.error('Validation Error Details:');
      if (error.response?.data?.errors) {
        console.table(error.response.data.errors);
          if (import.meta.env.DEV) console.table(error.response.data.errors);
      } else if (error.response?.data?.fieldErrors) {
        console.table(error.response.data.fieldErrors);
          if (import.meta.env.DEV) console.table(error.response.data.fieldErrors);
      } else if (error.response?.data?.message) {
        console.error('Validation message:', error.response.data.message);
          if (import.meta.env.DEV) console.error('Validation message:', error.response.data.message);
      ...
      try {
        ...
        console.log('Request payload that caused validation error:', requestData);
          if (import.meta.env.DEV) console.log('Request payload that caused validation error:', requestData);
        const missingRequiredFields: string[] = [];
        if (error.config?.url?.includes('/api/fees/payments')) {
          console.log('Validating payment request payload:');
            if (import.meta.env.DEV) console.log('Validating payment request payload:');
          ...
          if (missingRequiredFields.length > 0) {
            console.error('Missing required fields:', missingRequiredFields);
              if (import.meta.env.DEV) console.error('Missing required fields:', missingRequiredFields);
          }
        }
      } catch (e) {
        console.log('Request payload (could not parse):', error.config?.data);
          if (import.meta.env.DEV) console.log('Request payload (could not parse):', error.config?.data);
      }
    }

    // 401s are handled by the refresh interceptor registered in tokenRefreshService
