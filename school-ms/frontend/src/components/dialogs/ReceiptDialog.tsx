...
import { Receipt as ReceiptIcon, Print as PrintIcon, Email as EmailIcon } from '@mui/icons-material';
...
import { settingsService, SchoolSettings } from '../../services/settingsService';
  ...
  const ReceiptDialog: React.FC<ReceiptDialogProps> = ({
    ...
    useEffect(() => {
      (async () => {
        try {
          const s = await settingsService.getSchoolSettings();
          setSchool(s);
        } catch {
          // Non-blocking if settings fetch fails
        }
      })();
    }, []);
    ...
    const handlePrint = useReactToPrint({
      ...
      // @ts-ignore - content prop omitted in types
    ...
    const handleSendEmail = async () => {
      ...
      try {
        ...
        await notificationService.sendFeeReceiptEmail(studentId, payment.id!, emailTo);
        setNotification({ open: true, message: 'Receipt has been sent successfully to ' + emailTo, severity: 'success' });
        setShowEmailForm(false);
      } catch (error) {
        console.error('Failed to send receipt email:', error);
        setNotification({ open: true, message: 'Failed to send email. Please try again.', severity: 'error' });
      } finally {
  ...
  const receiptNumber = payment.receiptNumber || `R${String(payment.id ?? '').padStart(6, '0')}`;
  const amountNumber = Number(payment.amountPaid ?? payment.amount ?? 0);
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  <Box>
                    ...
                    {school?.phone && (
                      <Typography variant="body2" color="textSecondary">Phone: {school.phone}</Typography>
                    )}
                    {school?.email && (
                      <Typography variant="body2" color="textSecondary">Email: {school.email}</Typography>
                    )}
                  </Box>
                ...
                <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <Typography variant="h6">TOTAL PAID: ₹{amountNumber.toLocaleString()}</Typography>
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  <Typography variant="body2">
                    ...
                    {payment.frequency && frequencyLabels[payment.frequency]
                  ...
                  <Box sx={{ backgroundColor: '#f5f5f5', p: 2, borderRadius: 1, mt: 2 }}>
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={4} sx={{ textAlign: 'right' }}>
                        <Typography variant="subtitle1">₹{amountNumber.toLocaleString()}</Typography>
                      </Grid>
                    </Grid>
                  </Box>
                </Grid>
                ...
                <Grid item xs={12} sx={{ mt: 4 }}>
                  <Divider />
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 3, alignItems: 'flex-end' }}>
                    <Box sx={{ maxWidth: '50%' }}>
          ...
          {showEmailForm && (
            <Box sx={{ mt: 2, p: 2, border: '1px solid #eee', borderRadius: 1 }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                <Button size="small" onClick={() => setShowEmailForm(false)} sx={{ mr: 1 }}>
                  Cancel
                </Button>
                <Button size="small" variant="contained" onClick={handleSendEmail} disabled={!emailTo || sendingEmail}>
        ...
        <DialogActions sx={{ justifyContent: 'space-between', p: 2 }}>
          ...
          <Box>
            {!showEmailForm && studentId && (
              <Button startIcon={<EmailIcon />} onClick={() => setShowEmailForm(true)} sx={{ mr: 1 }}>
                Email Receipt
              </Button>
            )}
